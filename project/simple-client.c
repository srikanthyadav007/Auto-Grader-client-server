...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    fprintf(stderr, "usage %s hostname port file.c\n", argv[0]);
    exit(0);
  }
...
//convert host order port number to network order
  ...
  */
  ...
  /* ask user for input */
  char content[10000]; //To store the content of file
  int fd1=open(argv[3],O_RDONLY);
  int cnt_size = read(fd1,content,10000);
  ...
  */
  ...
  n = write(sockfd, content, cnt_size);
